/*
 * @Author: liming 
 * @Date: 2022-05-18 15:17:48 
 * @Last Modified by: liming
 * @Last Modified time: 2023-05-26 17:16:11
 * 
//调用方法
...
var GLOBAL_DATA = {
  ...
  afId: getUrlParam("afId"),
  gpsAdid: getUrlParam("gpsAdid"),
  appVersionV: getUrlParam("appVersionV"),
  androidId: getUrlParam("androidId"),
};

var TOAST_DIV_ID = "div-toast-tips";
var LOADING_DIV_ID = "div-global-loading";
var cjs = {}; // 原生调用js方法都放在这个类中

var TOAST_TIME = 1500; //ms
const SIGN_KEY = "2D4A29C2B37146396AE721EDFAB35FB8B42DA82F";

/**
 *
 * @param {*} path 本地html文件名
 * @param {*} isOpenNewWebview 是否以新的webview打开
 * @param {*} params 传递的参数
 */
function jumpLocalPage(path, params) {
  var params = $.extend({}, params, GLOBAL_DATA);
  var jumpUrl = path + "?" + $.param(params);
...
function setAppLogin(data) {
  ...
  GLOBAL_DATA.userId = data.userId;
  GLOBAL_DATA.phone = data.phone;
  try {
    wjs.registAndLoginTo(
      GLOBAL_DATA.token,
      GLOBAL_DATA.userId,
      GLOBAL_DATA.phone
    );
...
function showLoading() {
  var $loading = $("#" + LOADING_DIV_ID);
  if (!$loading.length) {
    $("body").append(
      `<div id="${LOADING_DIV_ID}" class="global-loading"><img src="./images/loading.gif" /></div>`
    );
    $loading = $("#" + LOADING_DIV_ID);
  }
  $loading.show();
}
...
function showToast(text) {
  ...
  $("body").append(
    '<div id="' +
      TOAST_DIV_ID +
      '"style="position: fixed; border-radius: 5px; background: rgba(0, 0, 0, 0.7); z-index: 999; text-align: center; top: 50%; left: 50%; padding: 10px 14px; transform: translate(-50%, -50%); font-size: 12px; line-height: 20px; font-weight: 500; color: #fff;">' +
      text +
      "</div>"
  );
  $toast = $("#" + TOAST_DIV_ID);
...
function ajaxPost(url, param, successfn, errorfn) {
  ajax(url, "post", "json", param, successfn, errorfn);
}

//发送ajax post请求
function ajaxGet(url, param, successfn, errorfn) {
  ajax(url, "get", "json", param, successfn, errorfn);
}
...
function ajax(url, type, dataType, param, successfn, errorfn) {
  ...
  $.ajax({
    ...
    success: function (d) {
      console.log(d);
      if (d.returnCode != 2000) {
        try {
          wjs.onWebError(d.returnCode, d.message);
        } catch (error) {}
      }
      successfn(d);
    },
